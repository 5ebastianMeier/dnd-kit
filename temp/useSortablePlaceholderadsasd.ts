...
import {InternalContext} from 'packages/core/src/store';
import {useResizeObserver} from 'packages/core/src/hooks/utilities';
...
export function useSortable({
  ...
  const {activeNodeRect, active, activatorEvent} = useContext(InternalContext);
  ...
  const transform: Transform | null = useContext(ActiveDraggableContext);
  ...
  const finalTransform = displaceItem ? displaceTransform : null;
  const newIndex =
